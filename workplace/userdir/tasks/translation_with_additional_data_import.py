...
def import_user_module(module_path, package=None):
    ...
    if module_path is not None:
        ...
        if module_name not in sys.modules:
            sys.path.insert(0, module_parent)
            mdl = importlib.import_module(module_name, package)
            # sys.path.pop(0)
        else:
            mdl = importlib.import_module(module_name, package)
    return mdl
...
def load_langpair_with_additional_data_dataset(
    ...
    if add_dataset:
        usermdl = import_user_module(userdir)
        print('APPLE')
        import sys; print(sys.path)
        print(userdir)
        print(usermdl)
        return usermdl.data.LanguagePairWithAdditionalDataDataset(
...
@register_task('translation_with_additional_data_import')
class TranslationWithAdditionalDataImportTask(FairseqTask):
    """
    ...
    def load_dataset(self, split, epoch=0, combine=False, **kwargs):
        ...
        self.datasets[split] = load_langpair_with_additional_data_dataset(
            ...
            userdir=getattr(self.args, 'user_dir', None),
        )
